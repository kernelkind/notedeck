...
- **`crates/notedeck_columns`** – A multi-column micro-blogging app.
- **`crates/notedeck_ui`** – Reusable egui widgets across all apps: (`NoteView`, media renderers, profile components) and UI utilities.
- **`crates/notedeck_dave`** – Dave AI assistant showcasing agent-style tooling, streaming responses, and custom rendering.
- **`crates/tokenator`** – Text token utility library used for easy serialization.
...
- **`Notedeck` struct** (`crates/notedeck/src/app.rs`) owns global resources: NostrDB, caches, relay pool, accounts, zaps, localization, clipboard, frame history, and injects them through `AppContext`.
...
- **Database**: `nostrdb::Ndb` is the primary storage/query engine. Transactions are short-lived (`Transaction::new`) and most reads flow through caches. Nostrdb has made great efforts to be a zero-copy, O(1) access database. Data from ndb should NOT be cloned per frame.
- **Caches**:
  - `NoteCache`: store ndb note data here if necessary, instead of cloning per frame
  - `Images` (image/GIF cache),
  - `UnknownIds` system for fetching pubkeys & noteids from remote relays
...
- **Columns-specific layout**: `Damus` app (`crates/notedeck_columns/src/app.rs`) holds specific data related to the Columns app. It manages decks, per-column navigation routers, timeline hydration, and keyboard navigation. It uses `StripBuilder` and custom panels for multi-column flows.
...
## Nostrdb
`ndb::query` performs a query for the specific filter. Your code should not query the same filter over and over again for many frames. If a specific filter needs to be monitored over a long period of time, what should happen is `ndb::query` should occur on the first frame the filter is needed. The result should be cached. A `ndb::subscribe` should be performed, then on the next frames, `ndb::poll_for_notes` on that subscription should be used. `poll_for_notes` only retrieves notes which were ingested into `ndb` (ie, notes just received from relays) since the subscription was started.
