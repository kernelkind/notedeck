...
impl<'a> From<&'a WsEvent> for RelayEvent<'a> {
    fn from(event: &'a WsEvent) -> RelayEvent<'a> {
        match event {
            ...
            WsEvent::Error(err) => RelayEvent::Error(Error::WebSocket(err.clone())),
        }
    }
}
...
mod tests {
    ...
    fn websocket_error_event_preserves_error_message_and_raw_os_error() {
        let websocket_error =
            crate::WebSocketError::from(std::io::Error::from_raw_os_error(libc::EMFILE))
                .with_context("Connect");
        let websocket_event = WsEvent::Error(websocket_error.clone());
