...
pub(in crate::relay::outbox) enum FullHistoryNegentropyStartOutcome {
    /// The relay session was retained and its `NEG-OPEN` was queued.
    Started,
    /// The prepared relay leg is unusable; drop it from this local-set attempt.
    Drop,
    /// Relay capacity is not available yet; retain this relay and its completed
    /// local set for a later attempt.
    Retry,
}
...
/// missing ids.
///
/// Each active id retains the relay/filter target asked to supply it. Storage
/// confirmation clears all fetch state for that id; a timeout retries or fails
/// only that target, leaving alternate relay candidates eligible.
