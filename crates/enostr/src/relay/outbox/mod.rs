...
/// Application-level websocket ping/pong is a secondary liveness backstop.
/// TCP keepalive detects a silently dropped socket in about nine seconds and
/// the network watcher reacts immediately to route loss. Neither detects an
/// application-layer failure behind a still-live proxy, such as a wedged relay
/// process. Ping traffic also refreshes websocket-aware proxy idle timers.
...
mod tests {
    ...
    ///
    /// `ensure_relay` can build `WebsocketConn`; the native transport needs a
