...
/// One live websocket leg plus service-owned liveness and route metadata.
pub(super) struct ServiceWebsocketLeg {
    ...
    /// Local address of the connected socket, populated by `WsEvent::Opened`.
    /// This ties the leg to its network interface so route loss closes only
    /// connections using the lost subnet.
...
pub(super) struct RelayReconnectState {
    /// Open attempts in the current reconnect streak. A successful connection
    /// removes this state and therefore resets the count.
    pub(super) attempt: u32,
    /// Earliest time at which this relay may be opened again.
