...
///
/// Each request carries an attempt number so a completion from a superseded
/// attempt cannot update current relay limits. Failures enter bounded retry,
/// accepted limits enter the refresh interval, and temporarily deferred limit
/// application receives a short retry without being treated as an HTTP failure.
