...
impl<'a> CompactionTransitionState<'a> {
    ...
    /// Repack retired request filters into new compaction REQs.
    ///
    /// Projection is deliberate: a subscription that already caught up through
    /// compaction keeps its synthetic `since` cursor in the rebuilt wire REQ,
    /// while its stored filters remain unchanged.
