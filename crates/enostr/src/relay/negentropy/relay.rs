...
/// Outcome of attempting to start one relay-local negentropy session after
/// capacity has already been granted.
pub(crate) enum NegentropyStartResult {
    /// The session was retained and the returned `NEG-OPEN` should be sent.
    Started(ClientMessage),
    /// The session was not retained; return the caller-owned pass and drop this
    /// prepared request.
    Rejected(SubPass),
}
