...
use crate::{run_media_job_pre_action, JobPool};
use crate::{Error, JobsCache, MediaJobResult};
use crate::{MediaJobKind, NotedeckOptions};
use egui::Margin;
...
pub struct Notedeck {
    ...
    job_pool: JobPool,
    media_jobs: JobsCache<MediaJobKind, MediaJobResult>,
    i18n: Localization,
...
impl eframe::App for Notedeck {
    fn update(&mut self, ctx: &egui::Context, frame: &mut eframe::Frame) {
        ...
        self.media_jobs.run_received(&mut self.job_pool, |id| {
            run_media_job_pre_action(id, &mut self.img_cache.textures);
        });
...
impl Notedeck {
    ...
    pub fn new<P: AsRef<Path>>(ctx: &egui::Context, data_path: P, args: &[String]) -> Self {
        ...
        let media_job_cache = JobsCache::new(receive_new_jobs, send_new_jobs);
