...
pub fn fetch_invoice_promise(
    ...
    let (url, lnurl) = match zap_address {
        ZapAddress::Lud16(lud16) => {
            let url = generate_endpoint_url(&lud16).ok()?;
            let lnurl = endpoint_url_to_lnurl(&url).ok()?;
            (url, lnurl)
        }
        ZapAddress::Lud06(lnurl) => (convert_lnurl_to_endpoint_url(&lnurl).ok()?, lnurl),
    };

    match cache.get_response(&url) {
        Some(endpoint_resp) => {
            let response = endpoint_resp.clone();
            Some(Promise::spawn_async(tokio::spawn(async move {
        ...
        None => Some(Promise::spawn_async(tokio::spawn(async move {
...
async fn fetch_invoice_lnurl_async(
    ...
    let base_url = match &pay_entry.response.callback_url {
        UrlWrapper::Url(url) => url.clone(),
        UrlWrapper::DecodeFailed { raw: _, error } => {
            return FetchedInvoiceResponse {
