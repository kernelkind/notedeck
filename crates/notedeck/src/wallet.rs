...
impl Wallet {
    ...
    pub fn get_balance<'a>(&mut self, jobs: &'a mut Jobs) -> Option<&'a Result<u64, nwc::Error>> {
        ...
        let job_state = jobs.get_or_insert_with(&JobId::NWCBalance(&self.uri), None, move |_| {
            let balance = block_on(async { wallet.read().await.get_balance().await });
