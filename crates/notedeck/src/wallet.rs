...
pub fn get_wallet_for_mut<'a>(
    ...
    if let Some(wallet) = &mut cur_acc.wallet {
        return Some(&mut wallet.wallet);
    }

    global_wallet.wallet.as_mut().map(|w| &mut w.wallet)
}
...
impl TokenSerializable for ZapWallet {
    fn parse_from_tokens<'a>(
        ...
        if let Some(msats) = m_default_zap {
            zap_wallet = zap_wallet.with_default_zap_msats(msats.msats);
        }
