use crate::media::blur::BlurCache;
use crate::media::gif::AnimatedImgTexCache;
use crate::media::images::ImageType;
use crate::media::latest::TrustedMediaLatestTex;
use crate::media::static_imgs::StaticImgTexCache;
use crate::media::{AnimationMode, NoLoadingLatestTex, UntrustedMediaLatestTex};
...
pub enum LoadableTextureState<'a> {
    Pending,
    Error(&'a crate::Error),
    Loading { actual_image_tex: &'a TexturedImage }, // the texture is in the loading state, for transitioning between the pending and loaded states
    Loaded(&'a TexturedImage),
}
...
impl<'a> From<&'a mut TextureStateInternal> for LoadableTextureState<'a> {
    fn from(value: &'a mut TextureStateInternal) -> Self {
        match value {
            TextureStateInternal::Pending => LoadableTextureState::Pending,
