...
impl AccountStorage {
    ...
    pub fn add_account(&self, account: &UserAccount) -> Result<()> {
...
mod tests {
    ...
    fn test_basic() {
        ...
        let resp = storage.add_account(&UserAccount::new(kp.clone()));
    ...
    fn test_select_key() {
        ...
        let storage = AccountStorage::mock().unwrap();
        let _ = storage.add_account(&UserAccount::new(kp.clone()));
        assert_num_storage(&storage.get_accounts(), 1);
