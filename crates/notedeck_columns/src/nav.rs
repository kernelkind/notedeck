            ...
            EditProfileView::new(ctx.i18n, state, ctx.img_cache, ctx.clipboard)
                ...
                .map_output_maybe(|o| {
                    if let Some(state) = app.view_state.pubkey_to_profile_state.get(kp.pubkey) {
                        Some(RenderNavAction::ProfileAction(ProfileAction::SaveChanges(
...
impl<R> BodyResponse<R> {
    ...
    pub fn map_output<S>(self, f: impl FnOnce(R) -> S) -> BodyResponse<S> {
        BodyResponse {
            ...
            output: if let Some(out) = self.output {
                Some(f(out))
            } else {
                None
            },
    ...
    let nav_response = nav
        ...
        .show_mut(ui, |ui, render_type, nav| {
            let response = match render_type {
                NavUiType::Title => BodyResponse::output(
                    NavTitle::new(
                        ctx.ndb,
                        ctx.img_cache,
                        get_active_columns_mut(ctx.i18n, ctx.accounts, &mut app.decks_cache),
            ...
            let can_take_drag_from = if let Some(drag_id) = response.drag_id {
                vec![drag_id]
            } else {
                Vec::new()
            };
