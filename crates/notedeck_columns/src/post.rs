...
impl TextBuffer for PostBuffer {
    ...
    fn insert_text(&mut self, text: &str, char_index: usize) -> usize {
        println!("inserting text: {text} at index: {char_index}");
        if text.is_empty() {
            return 0;
        }
        ...
        // Step 1: Collect keys to modify (avoiding direct mutation during iteration)
        let keys_to_update: Vec<usize> = self
            ...
            .map(|(&k, _)| k)
            .collect();
        println!("keys_to_update: {:?}", keys_to_update);

        // Step 2: Modify `mention_ends` in a separate pass
        for cur_end in keys_to_update {
            if let Some(mention_index) = self.mention_ends.remove(&cur_end) {
                ...
                println!("Removed mention_ends {cur_end} and replaced with {new_end}");
                ...
                if let Some(mention_info) = self.mentions.get_mut(mention_index) {
                    if mention_info.start_index >= char_index {
                        ...
                        println!(
                            "Removed mention_starts {} and replaced with {new_start}",
                            mention_info.start_index
                        );
                    }
        ...
        if let Some(char) = text.chars().next() {
            if char == self.mention_indicator {
                ...
                println!("Pushing mention with position {start_index}, {end_index}",);
                ...
                println!("mentions: {:?}", self.mentions);
            }
        }
    ...
    fn delete_char_range(&mut self, char_range: Range<usize>) {
        ...
        println!("Deleting text from index {} to {}", start, end);
        self.text_buffer.drain(start..end);

        // Step 1: Collect affected mentions
            ...
            if let Some(mention_index) = self.mention_ends.remove(&cur_end) {
                if let Some(mention_info) = self.mentions.get_mut(mention_index) {
                    ...
                    if mention_info.start_index >= start && mention_info.end_index <= end {
                        ...
                        println!("Removed mention at index {}", mention_index);
                        continue;
                    }
                    ...
                    if mention_info.start_index >= end {
                        ...
                        println!(
                            "Updated mention start index to {}",
                            mention_info.start_index
                        );
                    }
                    ...
                    println!("Updated mention end index to {}", mention_info.end_index);
                } else {
                    error!("Could not find mention at index {}", mention_index);
                }
            }
