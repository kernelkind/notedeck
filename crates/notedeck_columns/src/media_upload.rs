...
fn internal_nip96_upload(
    ...
    ehttp::fetch(request, move |response| {
            ...
            Ok(response) => {
                if response.ok {
                    match String::from_utf8(response.bytes.clone()) {
                        Ok(str_response) => {
                            match serde_json::from_str::<serde_json::Value>(&str_response) {
                                Ok(v) => {
                                    ...
                                    match serde_json::from_value::<Vec<Vec<String>>>(tags) {
                                        Ok(tags) => {
                                            Nip94Event::from_tags_and_content(tags, content)
        ...
        println!("Uploaded media status: {:?}", maybe_uploaded_media);
...
mod tests {
    ...
    use crate::{
        images::fetch_binary_from_disk,
        media_upload::{
            get_nostr_build_upload_url, get_upload_url_from_provider, nostrbuild_nip96_upload,
            MediaPath, NOSTR_BUILD_URL,
        },
        test_utils::promise_wait,
    };
    ...
    async fn test_file() {
        let file_path = PathBuf::from_str("../../assets/damus_rounded_80.png").unwrap();
        let file_path = fs::canonicalize(file_path).unwrap(); // make absolute
        assert!(file_path.is_file());
        assert!(file_path.is_absolute());
        let media_path = MediaPath::new(file_path).unwrap();

        let res = fetch_binary_from_disk(media_path.full_path.clone());

        // let res = promise_wait(&promise);
