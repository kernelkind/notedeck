...
use crate::ui::anim::{AnimationHelper, ICON_EXPANSION_MULTIPLE};
        ...
        for (i, media) in self.draft.uploaded_media.iter().enumerate() {
            ...
            match self.img_cache.map()[&media.url].ready() {
                Some(Ok(texture)) => {
                    ...
                    if add_remove_upload_button(ui, remove_button_rect).clicked() {
                        to_remove.push(i);
                    }
...
fn add_remove_upload_button(ui: &mut egui::Ui, desired_rect: egui::Rect) -> egui::Response {
