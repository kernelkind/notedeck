...
fn deserialize_columns(ndb: &Ndb, deck_user: &[u8; 32], serialized: Vec<Vec<String>>) -> Columns {
    ...
    for serialized_routes in serialized {
        ...
        for (index, serialized_route) in serialized_routes.iter().enumerate() {
            println!("index {} serialized route: {}", index, serialized_route);
...
fn selections_to_route(selections: Vec<Selection>) -> Option<RouteIntermediary> {
    match selections.first()? {
        ...
        Selection::Keyword(Keyword::Notifs) => {
            println!("Got notifs keyword");
            match selections.get(1)? {
                Selection::Keyword(Keyword::Explicit) => {
                    println!("Got explicit keyword");
                    if let Selection::Payload(hex) = selections.get(2)? {
                        println!("return route intermediary");
...
mod tests {
    ...
    fn test_deserialize_columns() {
        ...
        assert_eq!(
            *router.routes().first().unwrap(),
            Route::Timeline(TimelineRoute::Timeline(TimelineId::new(0)))
        );
        ...
        assert_eq!(
            *router.routes().first().unwrap(),
            Route::Timeline(TimelineRoute::Timeline(TimelineId::new(1)))
        );
    }
}
